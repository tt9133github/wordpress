...
//     }
    ...
    public static function wp_print_footer_scripts(){
        ...
        if(!$xunhuweb_cron){
            $xunhuweb_cron=true;
            ?><script type="text/javascript">if(jQuery){jQuery(function($){$.ajax({url: '<?php echo XH_Social::instance()->ajax_url('xunhuweb_cron',false,false)?>',type: 'post',timeout: 60 * 1000,dataType: 'jsonp',async: true,cache: false});});}</script><?php
        }
    }
